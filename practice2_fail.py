...
from models.crc_test_v71 import crc_test_v71
from models.crc_test_v73 import crc_test_v73
from models.crc_test_v74 import crc_test_v74
...
iter =  1 # Iteration number for test outputs
title_ft = 23
score_ft = 28
...
# Config - test_outputs 디렉토리 경로와 모델 클래스 정보 (모델명 변경)
models_config = {
    'Dwarf-Net': {
        'class': crc_test_v72,
        ...
        'display_name': 'Dwarf-Net'
    },
    'Dwarf-Net_2skip':{
        'class': crc_test_v71,
        'test_outputs_dir': f'/userHome/userhome2/donghee/modelcombination/_output/output_{data_type}/crc_test_v71_Iter_{iter}/test_outputs',
        'display_name': 'Dwarf-Net_2skip'
    },
    'Dwarf-Net_3skip':{
        'class': crc_test_v73,
        'test_outputs_dir': f'/userHome/userhome2/donghee/modelcombination/_output/output_{data_type}/crc_test_v73_Iter_{iter}/test_outputs',
        'display_name': 'Dwarf-Net_3skip'
    },
    'Dwarf-Net_4skip':{
        'class': crc_test_v74,
        'test_outputs_dir': f'/userHome/userhome2/donghee/modelcombination/_output/output_{data_type}/crc_test_v74_Iter_{iter}/test_outputs',
        'display_name': 'Dwarf-Net_4skip'
    }
}
...
def analyze_large_polyp_performance(model_predictions, images_base_dir, masks_base_dir, focus_on_large=True):
    """
    큰 용종에서 Dwarf-Net의 성능 저하를 분석하는 함수
    focus_on_large=True: 큰 용종에서 Dwarf-Net이 다른 모델보다 성능이 떨어지는 경우를 찾음
    focus_on_large=False: 모든 용종 크기별 성능 분석
    """
    print(f"\n=== Analyzing Dwarf-Net performance on large polyps ===")
    
    # 모든 모델에 공통으로 있는 이미지들 찾기
    all_models = list(model_predictions.keys())
    if 'Dwarf-Net' not in all_models:
        print(f"Error: Dwarf-Net model not found! Available models: {all_models}")
        return []
    
    # 모든 모델에 예측이 있는 이미지들만 고려
    common_images = set(model_predictions['Dwarf-Net'].keys())
    for model_name in all_models:
        if model_name != 'Dwarf-Net':
    ...
    # 용종 크기별 통계
    size_stats = {
        "Small": {"count": 0, "dwarf_net_best": 0, "dwarf_net_worst": 0},
        "Medium": {"count": 0, "dwarf_net_best": 0, "dwarf_net_worst": 0},
        "Large": {"count": 0, "dwarf_net_best": 0, "dwarf_net_worst": 0},
        "Very Large": {"count": 0, "dwarf_net_best": 0, "dwarf_net_worst": 0}
    }
    ...
    for i, img_name in enumerate(common_images):
        ...
        try:
            ...
            # Dwarf-Net 성능 분석
            dwarf_net_score = all_scores['Dwarf-Net']
            other_scores = [score for model, score in all_scores.items() if model != 'Dwarf-Net']
            max_other_score = max(other_scores) if other_scores else 0.0
            min_other_score = min(other_scores) if other_scores else 0.0
            
            # 성능 순위 계산
            all_score_pairs = [(model, score) for model, score in all_scores.items()]
            all_score_pairs.sort(key=lambda x: x[1], reverse=True)
            dwarf_net_rank = [i for i, (model, _) in enumerate(all_score_pairs) if model == 'Dwarf-Net'][0] + 1
            ...
            if best_model == 'Dwarf-Net':
                size_stats[polyp_size_category]["dwarf_net_best"] += 1
            if worst_model == 'Dwarf-Net':
                size_stats[polyp_size_category]["dwarf_net_worst"] += 1
            
            # 성능 차이 계산
            performance_gap = dwarf_net_score - max_other_score  # 양수면 Dwarf-Net이 더 좋음
            ...
            analyzed_images.append({
                ...
                'dwarf_net_score': dwarf_net_score,
                'max_other_score': max_other_score,
                'performance_gap': performance_gap,
                'dwarf_net_rank': dwarf_net_rank,
    ...
    # 용종 크기별 통계 출력
    print(f"\n=== Polyp Size Distribution and Dwarf-Net Performance ===")
    for size_category, stats in size_stats.items():
        count = stats["count"]
        if count > 0:
            best_rate = (stats["dwarf_net_best"] / count) * 100
            worst_rate = (stats["dwarf_net_worst"] / count) * 100
            print(f"{size_category:>10} polyps: {count:>3} images")
            print(f"           Dwarf-Net BEST:  {stats['dwarf_net_best']:>3}/{count} ({best_rate:>5.1f}%)")
            print(f"           Dwarf-Net WORST: {stats['dwarf_net_worst']:>3}/{count} ({worst_rate:>5.1f}%)")
    
    if not analyzed_images:
        print("No valid analyzed images found!")
        return []
    
    # 큰 용종에서 Dwarf-Net 성능이 떨어지는 경우 필터링
    if focus_on_large:
        # 큰 용종 (Large, Very Large)에서 Dwarf-Net이 다른 모델보다 성능이 떨어지는 경우
        large_polyp_poor_performance = [
            img for img in analyzed_images 
            if img['polyp_size_category'] in ['Large', 'Very Large'] 
            and img['performance_gap'] < 0  # Dwarf-Net이 다른 모델보다 성능이 떨어짐
        ]
        
        # 성능 차이가 큰 순으로 정렬 (Dwarf-Net이 가장 많이 떨어지는 순)
        large_polyp_poor_performance.sort(key=lambda x: x['performance_gap'])
        
        print(f"\n=== Large polyps where Dwarf-Net underperforms ===")
        print(f"Found {len(large_polyp_poor_performance)} large polyps where Dwarf-Net performs worse")
        
        selected_images = large_polyp_poor_performance[:5]  # 상위 5개
        ...
        for i, img_info in enumerate(selected_images):
            ...
            print(f"     Dwarf-Net: {img_info['dwarf_net_score']:.4f} (rank #{img_info['dwarf_net_rank']})")
...
def create_size_performance_plot(analyzed_images):
    ...
    for idx, size_cat in enumerate(size_categories):
        ...
        colors = ['red' if model == 'Dwarf-Net' else 'skyblue' for model in model_names]
...
# 큰 용종에서 Dwarf-Net 성능 저하 분석
if 'Dwarf-Net' not in model_predictions or len(model_predictions['Dwarf-Net']) == 0:
    available_models = list(model_predictions.keys())
    print(f"Error: Dwarf-Net model predictions not found!")
...
if not selected_images:
    print("No large polyps found where Dwarf-Net underperforms!")
    exit()
...
for col, title in enumerate(column_titles):
    if title == "Dwarf-Net":  # Dwarf-Net 강조 (성능 저하 사례이므로 다른 색상)
...
for row, res in enumerate(results):
    ...
    for col, (model_name, pred) in enumerate(res['preds'].items()):
        ...
        # DSC 점수 표시
        if model_name == 'Dwarf-Net':
            # Dwarf-Net은 성능이 떨어지는 사례이므로 빨간색으로 강조
...
plt.savefig(save_dir+"large_polyp_dwarf_net_underperformance.png", dpi=300, bbox_inches='tight')
plt.close()

print("\n✓ Saved large_polyp_dwarf_net_underperformance.png")

# 성능 통계 출력 (큰 용종에서 Dwarf-Net 성능 저하 사례들)
print("\n=== Performance Statistics for Large Polyps where Dwarf-Net Underperforms ===")
for model_name in models_config.keys():
...
if large_cases:
    ...
    # Dwarf-Net과 최고 성능 모델 비교
    dwarf_net_scores = [res['dscs']['Dwarf-Net'] for res in large_cases]
    ...
    print(f"  Average Dwarf-Net score: {np.mean(dwarf_net_scores):.4f}")
...
print(f"\nProcessed {len(results)} large polyp cases where Dwarf-Net underperforms!")
print("These cases show Dwarf-Net struggling with larger polyps")
print("Check large_polyp_dwarf_net_underperformance.png for visual comparison")
