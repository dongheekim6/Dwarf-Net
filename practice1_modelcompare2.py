...
iter =4# Iteration number for test outputs
title_ft = 28
score_ft = 28
...
if len(loaded_models) < 2:
    ...
    if common_base_names:
        ...
        selected_images = []
        selected_info = []
        for i, img_info in enumerate(small_polyp_best_proposed[:15]):
...
# Visualization도 results_top10 기준
n_models = len(models_config) + 2
fig, axes = plt.subplots(len(results_top10), n_models, figsize=(3*n_models, 6*len(results_top10)))

if len(results_top10) == 1:
    axes = np.expand_dims(axes, 0)

# 모델 이름 리스트: 원본, GT, 그리고 각 모델
col_labels = ['Original', 'GT'] + [model for model in models_config.keys()]

for row, res in enumerate(results_top10):
    img_show = res['img']

    print("img_show dtype:", img_show.dtype)
    print("img_show shape:", img_show.shape)
    print("img_show min/max:", img_show.min(), img_show.max())
    if img_show.ndim == 2:
        print("❗️이미지가 2D입니다. 컬러 정보가 없습니다.")
    elif img_show.shape[2] != 3:
        print("❗️이미지 채널 수가 3이 아닙니다:", img_show.shape)

   
    print("show before shape", img_show.shape)
    axes[row, 0].imshow(img_show)  # cmap 지정하지 않음
    axes[row, 0].axis('off')
    if row == 0:
        axes[row, 0].set_title(col_labels[0], fontsize=score_ft, fontweight='bold')
        axes[row, 1].set_title(col_labels[1], fontsize=score_ft, fontweight='bold')
...
plt.savefig(save_dir + "comparison_test_outputs_test.png", dpi=300, bbox_inches='tight')
plt.close()
print(f"\n✓ Saved {save_dir}comparison_test_outputs_test.png")
...
print(f"Results saved to: {save_dir}comparison_test_outputs.png")
