...
class ImagesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sizes = 352
        if image.shape[-2:] != (sizes, sizes):
            image = F.interpolate(
                image.unsqueeze(0), size=(sizes, sizes),
                mode='bilinear', align_corners=False
            ).squeeze(0)

        if mask.shape[-2:] != (352, sizes):
            mask = F.interpolate(
                mask.unsqueeze(0), size=(sizes, sizes),
                mode='nearest'
            ).squeeze(0)
...
def Intersection_over_Union(yhat, ytrue, threshold=0.5):
    ...
    union = np.sum(yhat) + np.sum(ytrue) - intersection
...
def Do_Experiment(iteration, model_name, model, train_loader, validation_loader, test_loader, Optimizer, lr,  number_of_classes, epochs, Metrics,df,device, transform):
    ...
    return df
from models.CaraNet_v2 import CaraNet_v2
from models.haesung.DualRes2NetUNet_XFA_v4_1_4 import DualRes2NetUNet_XFA_v4_1_4
from models.haesung.DualRes2NetUNet_XFA_v4_1 import DualRes2NetUNet_XFA_v4_1
...
module_names = ['DualRes2NetUNet_XFA_v4_1']
...
#     Dataset_dir = f'splits_bkai/split{str(iteration).zfill(2)}'
...
#         # from models.PolypPVT import PolypPVT
#         # # model = convsegnet_v2(in_channels=in_channels, out_channels=number_of_classes)
#         # model = PolypPVT(in_Channel=3, out_channel=1)
...
for iteration in range(iterations[0], iterations[1]+1):
    ...
    # 각 iteration에 맞는 split 폴더 설정
    Dataset_dir = f'splits_colondb/split{str(iteration).zfill(2)}'
...
for iteration in range(iterations[0], iterations[1]+1):
    ...
    # 각 iteration에 맞는 split 폴더 설정
    Dataset_dir = f'splits/split{str(iteration).zfill(2)}'
...
for iteration in range(iterations[0], iterations[1]+1):
    ...
    # 각 iteration에 맞는 split 폴더 설정
    Dataset_dir = f'splits_kvasir_1000/split{str(iteration).zfill(2)}'
