...
def create_kvasir_splits(dataset_path, output_path, num_splits=10, train_ratio=0.8, val_ratio=0.1, test_ratio=0.1):
    ...
    for base_name in base_names:
        mask_file = masks_path / f"{base_name}.jpg"
        if mask_file.exists():
    ...
    for split_idx in range(1, num_splits + 1):
        ...
        def copy_files(pairs, subset_name):
            ...
            for base_name in pairs:
                # 이미지 파일 복사
                src_img = images_path / f"{base_name}.jpg"
                dst_img = subset_path / f"{base_name}.jpg"
                shutil.copy2(src_img, dst_img)
                
                # 마스크 파일 복사 (마스크는 _mask 접미사 추가)
                src_mask = masks_path / f"{base_name}.jpg"
...
if __name__ == "__main__":
    # 데이터셋 경로와 출력 경로 설정
    dataset_path = "/userHome/userhome2/donghee/modelcombination/sessile-main-Kvasir-SEG"  # Kvasir 데이터셋 경로 (images, masks 폴더가 있는 경로)
    output_path = "/userHome/userhome2/donghee/modelcombination/splits_kvasir_new_v3"  # 출력 경로
